...
!###################################################################################
     ...
     !the average for the day before "m" are calculated as 0.  eg for day 10, sum (day 1 to day 10)/10
     ...
     implicit none
         ...
         if (m>n) then  !bypass routine if window is bigger than data
            listout = -99.  
            return
         end if
       ...
       !  forall(i=m:n)   listout(i) = sum(list(i-m+1:i))/dble(m) !calc for days with enough previous data
         forall(i=1:m-1) listout(i) = 0.0   !calculation for first days with insufficient data for complete window
